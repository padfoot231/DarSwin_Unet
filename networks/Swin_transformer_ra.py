...
class FinalPatchExpand_X4(nn.Module):
    def __init__(self, input_resolution, dim, input_dim=96, norm_layer=nn.LayerNorm, n_radius=10, n_azimuth=10):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
...
class SwinTransformerRa(nn.Module):
    ...
    def __init__(self, img_size=224, radius_cuts=16, azimuth_cuts = 64, in_chans=3, num_classes=1000,
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer ==0 :
                ...
                patches_resolution[1] // (2 ** (1*(self.num_layers-1-i_layer)))), dim=int(embed_dim * 2 ** (self.num_layers-1-i_layer)), dim_scale=2, norm_layer=norm_layer)
            else:
                layer_up = BasicLayer_up(dim=int(embed_dim * 2 ** (self.num_layers-1-i_layer)),
                                input_resolution=(patches_resolution[0] // (2 ** (self.num_layers-1-i_layer)),
                                                    patches_resolution[1] // (2 ** (1*(self.num_layers-1-i_layer)))),
                                patch_size = patch_size,
    ...
    def forward_features(self, x, dist, label):
        ...
        for layer in self.layers:
            x_downsample.append([x, D_s]) # the last value of swin_downsample is output of first bottleneck
            x, D_s = layer(x, D_s)
        x = self.norm(x)  # B L C
    ...
    def forward_up_features(self, x, D_s, x_downsample):
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                ...
                #concat dim later, skip connections
...
if __name__=='__main__':
    ...
    model = model.cuda()
    # 
    t = torch.ones(1, 3, 128, 128).cuda("cuda:0")
    D = torch.tensor([100, 10, 10, 1]).reshape(4,1).transpose(1,0).cuda("cuda:0")
    dist = torch.tensor([0.5, 17.51, 3.047911227757854]).reshape(3, 1).transpose(0,1).cuda("cuda:0")
    l = torch.ones(1, 128, 128).cuda("cuda:0")
