...
def get_inverse_dist_spherical(num_points, xi, fov, new_f):
    # import pdb;pdb.set_trace()
    # xi = torch.tensor(xi).cuda("cuda:0")
    # width = torch.tensor(width).cuda("cuda:0")
    # # focal_length = torch.tensor(focal_length).cuda("cuda:0")
...
#     theta_d_max = inverse_rad(new_f)
    ...
    # theta_d_num1 = linspace(t1, t2, 10).cuda("cuda:0")
    r_list = rad(theta_d)   
